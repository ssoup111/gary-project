...
export default function AdminPage() {
  ...
  async function searchStockPhotos() {
    if (!stockQuery.trim()) {
      setStockStatus("Enter a search term first.");
      return;
    }
    setStockStatus("Searching...");
    setStockPhotos([]);
    const res = await fetch(
      `/api/admin/stock-search?query=${encodeURIComponent(stockQuery)}&source=${stockSource}`
    );
    const result = await res.json();
    if (!result.success) {
      setStockStatus(result.error || "Search failed.");
      return;
    }
    ...
    setStockStatus(
      result.photos.length === 0
        ...
        : `Found ${result.photos.length} photos.`
    );
  }

  async function importStockPhoto(photo: StockPhoto) {
    setImportingId(photo.id);
    setStockStatus(`Importing ${photo.source} photo...`);
    const res = await fetch("/api/admin/stock-import", {
      ...
      body: JSON.stringify({
        ...
        categoryId: stockCategoryId || null,
      }),
    });
    const result = await res.json();
    setImportingId(null);
    if (!result.success) {
      setStockStatus(result.error || "Import failed.");
      return;
    }
  ...
  function CategoryControls({ image }: { image: GeneratedImage }) {
    ...
    return (
      <div className="mt-4 rounded-xl border border-zinc-800 bg-zinc-950 p-3">
        ...
        {image.category_slug && (
          <p className="mt-2 text-xs text-amber-300">Current: {image.category_slug}</p>
        )}
      </div>
    );
  }
    ...
    <main className="min-h-screen bg-zinc-950 px-6 py-16 text-white">
      <div className="mx-auto max-w-6xl">
        ...
        <p className="mt-4 max-w-2xl text-zinc-400">
          Generate AI images or import stock photos, then approve them into the catalog.
        </p>
        ...
        <section className="mt-10 rounded-2xl border border-zinc-800 bg-zinc-900 p-6">
          ...
          <p className="mt-2 text-sm text-zinc-400">
            Search Pexels and Unsplash for real photographs to add to your review queue.
          </p>
          ...
          <div className="mt-6 grid gap-4 sm:grid-cols-[1fr_auto_auto]">
            <input
              ...
              placeholder="e.g. golden retriever, mountain sunset, faith cross"
              className="w-full rounded-xl border border-zinc-700 bg-zinc-950 p-3 text-white placeholder:text-zinc-600"
            />
            <select
              value={stockSource}
              onChange={(e) => setStockSource(e.target.value as "pexels" | "unsplash" | "both")}
          ...
          <div className="mt-4 flex flex-wrap items-center gap-4">
            <div>
              ...
              <select
                ...
                className="mt-1 block rounded-lg border border-zinc-700 bg-zinc-950 p-2 text-sm text-white"
              >
          ...
          {stockStatus && (
            <p className="mt-4 text-sm font-bold text-zinc-300">{stockStatus}</p>
          )}
                  ...
                  >
                    ...
                    <div className="p-3">
                      ...
                      <p className="mt-1 text-xs text-zinc-600">
                        ...
                        <span className={photo.source === "pexels" ? "text-green-400" : "text-blue-400"}>
                          {photo.source}
                        </span>
                      </p>
                      <button
                        ...
                        className={`mt-3 w-full rounded-lg px-3 py-2 text-xs font-black transition ${
                          isImported
                      ...
                      >
                        {isImported ? "✓ Added to Queue" : isImporting ? "Importing..." : "Add to Review Queue"}
                      </button>
                    </div>
                  </div>
                );
            ...
            <div className="mt-6 grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
              {pendingImages.map((image) => (
                <div key={image.id} className="overflow-hidden rounded-2xl border border-zinc-800 bg-zinc-900">
                  {image.image_url && (
                    <img src={image.image_url} alt={image.prompt} className="h-72 w-full object-cover" />
                  )}
                  <div className="p-5">
                    <p className="text-sm leading-6 text-zinc-400">{image.prompt}</p>
                    ...
                    <div className="mt-5 flex flex-wrap gap-3">
                      ...
                      <button onClick={() => regenerateImage(image.prompt)} className="rounded-lg bg-zinc-700 px-4 py-2 text-sm font-bold text-white">Regenerate</button>
                    </div>
                  </div>
                </div>
              ))}
          ...
          {approvedImages.length === 0 ? (
            ...
            <div className="mt-6 grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
              {approvedImages.map((image) => (
                <div key={image.id} className="overflow-hidden rounded-2xl border border-zinc-800 bg-zinc-900">
                  {image.image_url && (
                    <img src={image.image_url} alt={image.prompt} className="h-56 w-full object-cover" />
                  )}
                  <div className="p-4">
                    <p className="text-xs leading-5 text-zinc-400">{image.prompt}</p>
