  ...
  function RecipientCard({ recipientId }: { recipientId: string | null }) {
    ...
    return (
      <div className="mt-3 rounded-xl border border-black/10 bg-white p-3">
        <p className="text-xs font-bold uppercase tracking-wider text-[#B31942]">Recipient</p>
        <p className="mt-1 font-black">{name || "—"}</p>
        {rec.offender_id && <p className="text-xs text-[#0A3161]/60">Inmate #: {rec.offender_id}</p>}
        {rec.facility && <p className="text-xs text-[#0A3161]/60">Facility: {rec.facility}</p>}
        {rec.state && <p className="text-xs text-[#0A3161]/60">State: {rec.state}</p>}
      </div>
    );
  }
  ...
  return (
    <main className="min-h-screen bg-white px-6 py-16 text-[#0A3161]">
      <div className="mx-auto max-w-5xl">
        ...
        <p className="mt-3 text-[#0A3161]/60">Your image purchases, package credits, and subscription plans.</p>
        ...
        {paymentStatus === "cancelled" && (
          <div className="mt-8 rounded-2xl border border-[#8f1434]/40 bg-[#8f1434]/10 p-5">
            ...
            <p className="mt-1 text-sm text-[#B31942]/70">You can <Link href="/order" className="underline">start a new order</Link> anytime.</p>
          </div>
        )}
        {statusMsg && <div className="mt-6 rounded-2xl border border-black/12 bg-white p-4 font-bold text-[#0A3161]/70">{statusMsg}</div>}
        ...
        {notSignedIn ? (
          <div className="mt-10 rounded-3xl border border-black/10 bg-white p-10">
            ...
            <Link href="/login" className="mt-6 inline-block rounded-xl bg-[#B31942] px-6 py-3 font-black text-white">Sign In</Link>
          </div>
        ) : loading ? (
          ...
          <div className="mt-10 rounded-3xl border border-black/10 bg-white p-10">
            ...
            <p className="mt-3 text-[#0A3161]/60">Browse the catalog and create your first order.</p>
            <div className="mt-6 flex gap-4">
              <Link href="/catalog" className="rounded-xl bg-[#B31942] px-6 py-3 font-black text-white hover:bg-[#8f1434]">Browse Catalog</Link>
              <Link href="/pricing" className="rounded-xl border border-black/12 px-6 py-3 font-black text-[#0A3161] hover:border-[#B31942]">View Pricing</Link>
            </div>
          </div>
        ) : (
          <>
            ...
            <div className="mt-8 flex gap-3">
              <button onClick={() => setTab("orders")}
                className={"rounded-full px-5 py-2 text-sm font-black transition " + (tab === "orders" ? "bg-[#B31942] text-white" : "border border-black/12 text-[#0A3161]/60 hover:border-[#B31942]")}>
                Images ({individualOrders.length})
              </button>
              <button onClick={() => setTab("subscriptions")}
                className={"rounded-full px-5 py-2 text-sm font-black transition " + (tab === "subscriptions" ? "bg-[#B31942] text-white" : "border border-black/12 text-[#0A3161]/60 hover:border-[#B31942]")}>
              ...
              <div className="mt-6 grid gap-5">
                {individualOrders.length === 0 ? (
                  <div className="rounded-3xl border border-black/10 bg-white p-10 text-center">
                    <p className="font-bold text-[#0A3161]/60">No single-image orders yet</p>
                    <Link href="/catalog" className="mt-4 inline-block rounded-xl bg-[#B31942] px-6 py-3 font-black text-white">Browse Catalog</Link>
                  </div>
                  ...
                  return (
                    <div key={order.id} className={"rounded-3xl border bg-white p-6 " + (isPending ? "border-[#8f1434]/40" : "border-black/10")}>
                      <div className="grid gap-6 lg:grid-cols-[180px_1fr_auto]">
                        <div>
                          {image?.image_url ? (
                            ...
                            <div className="flex h-44 items-center justify-center rounded-2xl border border-black/10 bg-white text-sm text-[#0A3161]/50">No image</div>
                          )}
                        </div>
                        <div>
                          <p className="text-lg font-black">Order #{order.id.slice(0, 8).toUpperCase()}</p>
                          <p className="mt-1 text-xs text-[#0A3161]/50">{new Date(order.created_at).toLocaleString()}</p>
                          {image && <p className="mt-3 line-clamp-3 text-sm leading-6 text-[#0A3161]/70">{image.prompt}</p>}
                          <RecipientCard recipientId={order.recipient_id} />
                        </div>
                        <div className="text-left lg:text-right">
                          <span className={"inline-block rounded-full px-3 py-1 text-xs font-bold uppercase " + (order.payment_status === "paid" ? "bg-green-100 text-green-700" : "bg-[#8f1434]/20 text-[#B31942]")}>
                            {order.payment_status === "paid" ? "Paid" : "Payment Pending"}
                          </span>
                          <p className="mt-2 text-xs text-[#0A3161]/50">{order.delivery_status || order.status}</p>
            ...
            {tab === "subscriptions" && (
              <div className="mt-6 grid gap-5">
                {subscriptions.length === 0 ? (
                  <div className="rounded-3xl border border-black/10 bg-white p-10 text-center">
                    <p className="font-bold text-[#0A3161]/60">No packages or subscriptions yet</p>
                    <Link href="/pricing" className="mt-4 inline-block rounded-xl bg-[#B31942] px-6 py-3 font-black text-white">View Plans</Link>
                  </div>
                  ...
                  const statusColors: Record<string, string> = {
                    ...
                    paused: "bg-[#8f1434]/20 text-[#B31942]",
                    cancelled: "bg-red-100 text-red-700",
                    completed: "bg-black/10 text-[#0A3161]/60",
                  };
                  const statusColor = statusColors[sub.status || ""] || "bg-[#F1F4F9] text-[#0A3161]/50";
                  return (
                    <div key={sub.id} className="rounded-3xl border border-black/10 bg-white p-6">
                      <div className="flex items-start justify-between gap-4">
                        <div>
                          ...
                          <p className="mt-1 text-xs text-[#0A3161]/50">{isDaily ? "Daily delivery plan" : "Image credits"} · Started {fmt(sub.start_date)}</p>
                        </div>
                      ...
                      <div className="mt-6">
                        <div className="flex items-end justify-between text-sm">
                          <p className="font-bold text-[#0A3161]/70">Images Remaining</p>
                          <p className="font-black"><span className="text-[#B31942] text-xl">{sub.images_remaining ?? "?"}</span><span className="text-[#0A3161]/45">/{sub.images_total}</span></p>
                        </div>
                        <div className="mt-2 h-2 w-full rounded-full bg-[#F1F4F9]">
                          <div className="h-full rounded-full bg-[#B31942] transition-all" style={{ width: `${100 - usedPct}%` }} />
                        </div>
                        <p className="mt-1 text-xs text-[#0A3161]/45">{usedPct}% used</p>
                      </div>

                      <div className="mt-5 grid gap-4 sm:grid-cols-2">
                        <div>
                          {isDaily && sub.next_delivery_date && (
                            <div className="rounded-xl border border-[#B31942]/20 bg-[#B31942]/5 p-3">
                              <p className="text-xs font-bold text-[#B31942]">Next Delivery</p>
                              <p className="mt-1 font-black">{fmt(sub.next_delivery_date)}</p>
                            </div>
                          )}
                          {sub.end_date && (
                            <p className="mt-2 text-xs text-[#0A3161]/50">Plan expires: {fmt(sub.end_date)}</p>
                          )}
                        </div>
                        <div>
                      ...
                      {!isDaily && (sub.images_remaining || 0) > 0 && (
                        <Link href={`/order?plan=single`} className="mt-5 inline-block rounded-xl bg-[#B31942] px-5 py-2.5 text-sm font-black text-white hover:bg-[#8f1434]">
                          Use a Credit →
                        </Link>
                      )}
                    </div>
