...
export default function MyOrdersPage() {
  ...
  async function loadOrders() {
    ...
    const { data: userRecipients } = await supabase
    ...
    (userRecipients || []).forEach((r) => { recipientMap[r.id] = r; recipientIds.push(r.id); });

    if (recipientIds.length === 0) { setLoading(false); return; }
    ...
    if (orderIds.length > 0) {
      ...
      const imageIds = Array.from(new Set((itemsData || []).map((i) => i.generated_image_id).filter(Boolean)));
  ...
  async function retryPayment(orderId: string) {
    ...
    setRetryingId(null);
    if (!result.success || !result.url) { setStatusMsg(result.error || "Failed to open checkout."); return; }
  ...
  return (
    <main className="min-h-screen bg-zinc-950 px-6 py-16 text-white">
      <div className="mx-auto max-w-5xl">
        ...
        {statusMsg && <div className="mt-6 rounded-2xl border border-amber-500 bg-amber-500/10 p-4 font-bold text-amber-300">{statusMsg}</div>}
                ...
                <div key={order.id} className={"rounded-3xl border bg-zinc-900 p-6 " + (isPending ? "border-amber-500/40" : "border-zinc-800")}>
                  <div className="grid gap-6 lg:grid-cols-[180px_1fr_auto]">
                    ...
                    <div>
                      ...
                      {image && <p className="mt-4 line-clamp-3 text-sm leading-6 text-zinc-300">{image.prompt}</p>}
                      <div className="mt-5 rounded-2xl border border-zinc-800 bg-zinc-950 p-4">
                        <p className="text-sm font-bold uppercase tracking-[0.15em] text-amber-300">Recipient</p>
                        <p className="mt-2 font-black">{recipient?.full_name || "Unknown"}</p>
                        <p className="mt-1 text-sm text-zinc-400">Inmate #: {recipient?.inmate_number || "Not provided"}</p>
                    ...
                    <div className="text-left lg:text-right">
                      ...
                      <p className="mt-3 text-sm text-zinc-400">{order.status}</p>
