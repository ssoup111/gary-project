      ...
      <div className="mx-auto max-w-4xl">
        ...
        <section className="mt-10 rounded-3xl border border-black/10 bg-white p-8">
          ...
          <div className="mt-6 grid gap-5">
            ...
            <div>
              <label className="block text-sm font-bold text-[#0A3161]/85">Inmate / DOC Number <span className="text-[#B31942]">*</span></label>
            ...
            <div>
              <label className="block text-sm font-bold text-[#0A3161]/85">Facility Name</label>
              <input value={facilityName} onChange={(e) => setFacilityName(e.target.value)} placeholder="e.g. Stateville Correctional Center" className="mt-2 w-full rounded-xl border border-black/12 bg-white p-3 text-[#0A3161] placeholder:text-[#0A3161]/55" />
            </div>
            <div>
              <label className="block text-sm font-bold text-[#0A3161]/85">State</label>
              <input value={state} onChange={(e) => setState(e.target.value)} placeholder="e.g. Illinois" className="mt-2 w-full rounded-xl border border-black/12 bg-white p-3 text-[#0A3161] placeholder:text-[#0A3161]/55" />
            </div>
            <div>
        ...
        <section className="mt-10">
          ...
          {loading ? (
            ...
            <div className="mt-6 rounded-2xl border border-black/10 bg-white p-8">
              <p className="text-[#0A3161]/78">No recipients saved yet. Add one above.</p>
            </div>
          ) : (
            <div className="mt-6 grid gap-5">
                ...
                <div key={recipient.id} className="rounded-3xl border border-black/10 bg-white p-6">
                  <div className="flex items-start justify-between gap-4">
                    <div>
                      ...
                      <p className="mt-2 text-[#0A3161]/78">Inmate #: {recipient.inmate_number || "Not provided"}</p>
                      <p className="text-[#0A3161]/78">Facility: {recipient.facility_name || "Not provided"}</p>
                      <p className="text-[#0A3161]/78">State: {recipient.state || "Not provided"}</p>
                      {recipient.notes && <p className="mt-3 text-sm text-[#0A3161]/72">{recipient.notes}</p>}
                    </div>
